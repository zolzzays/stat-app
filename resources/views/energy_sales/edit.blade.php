...
@section('title', 'Борлуулалтын мэдээ засах')

@section('content')

<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="mb-0 fw-bold">Борлуулалтын мэдээ засах</h4>
...
<form action="{{ route('energy_sales.update', $energy_sale->id) }}" method="POST">
    @csrf
    @method('PUT')
            ...
            <div class="row g-3">
                    ...
                    <select class="form-select" name="power_plant_id" required
                        ...
                        @foreach($plants as $plant)
                            <option value="{{ $plant->id }}"
                                {{ $plant->id == $energy_sale->power_plant_id ? 'selected' : '' }}>
                                {{ $plant->plant_name }}
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select" name="product_name" id="product_name" required>
                        <option value="">-- Сонгоно уу --</option>
                        <option value="Цахилгаан" {{ old('product_name', $energy_sale->product_name) === 'Цахилгаан' ? 'selected' : '' }}>Цахилгаан</option>
                        <option value="Дулаан"    {{ old('product_name', $energy_sale->product_name) === 'Дулаан'    ? 'selected' : '' }}>Дулаан</option>
                    </select>
                </div>
                ...
                <div class="col-md-1">
                    ...
                    <input type="text" class="form-control" name="unit_name" id="unit_name"
                           value="{{ old('unit_name', $energy_sale->unit_name) }}" required>
                </div>

                <div class="col-md-2">
                    <label class="form-label">Он <span class="text-danger">*</span></label>
                    <input type="number" class="form-control" name="year"
                           value="{{ old('year', $energy_sale->year) }}" min="2000" max="2100" required>
                </div>
                ...
                <div class="col-md-2">
                    ...
                    <select class="form-select" name="month" required>
                        @for($m = 1; $m <= 12; $m++)
                            <option value="{{ $m }}"
                                {{ old('month', $energy_sale->month) == $m ? 'selected' : '' }}>
                                {{ $m }} сар
                            </option>
                        @endfor
                    </select>
                ...
                <div class="row g-3">
                    <div class="col-md-3">
                        ...
                        <input type="number" step="0.01" min="0" class="form-control decimal-input" name="before_month"
                               value="{{ number_format(old('before_month', $energy_sale->before_month), 2, '.', '') }}" required>
                    </div>
                    <div class="col-md-3">
                        ...
                        <input type="number" step="0.01" min="0" class="form-control decimal-input" name="before_sal"
                               value="{{ number_format(old('before_sal', $energy_sale->before_sal), 2, '.', '') }}" required>
                    </div>
                </div>
            </div>
                ...
                <div class="row g-3">
                    <div class="col-md-3">
                        ...
                        <input type="number" step="0.01" min="0" class="form-control decimal-input" name="this_month"
                               value="{{ number_format(old('this_month', $energy_sale->this_month), 2, '.', '') }}" required>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Үнийн дүн<br>(мян.төг) <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" min="0" class="form-control decimal-input" name="this_sal"
                               value="{{ number_format(old('this_sal', $energy_sale->this_sal), 2, '.', '') }}" required>
                    </div>
                </div>
            </div>
                ...
                <div class="row g-3">
                    <div class="col-md-3">
                        ...
                        <input type="number" step="0.01" min="0" class="form-control decimal-input" name="year_usage"
                               value="{{ number_format(old('year_usage', $energy_sale->year_usage), 2, '.', '') }}" required>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Үнийн дүн<br>(мян.төг) <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" min="0" class="form-control decimal-input" name="year_sal"
                               value="{{ number_format(old('year_sal', $energy_sale->year_sal), 2, '.', '') }}" required>
                    </div>
                </div>
            </div>
                ...
                <div class="row g-3">
                    <div class="col-md-3">
                        ...
                        <input type="number" step="0.01" min="0" class="form-control decimal-input" name="this_musage"
                               value="{{ number_format(old('this_musage', $energy_sale->this_musage), 2, '.', '') }}" required>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Үнийн дүн<br>(мян.төг) <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" min="0" class="form-control decimal-input" name="this_msal"
                               value="{{ number_format(old('this_msal', $energy_sale->this_msal), 2, '.', '') }}" required>
                    </div>
                </div>
            </div>
